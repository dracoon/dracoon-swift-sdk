...
public class FileUpload: DracoonUpload {
    ...
    public func start() {
        self.createFileUpload(request: request, completion: { result in
            ...
            case .value(let response):
                ...
                self.startChunkedUpload(token: response.token)
    ...
    public func cancel() {
        ...
        if let token = self.token {
            self.deleteUpload(token: token, completion: { _ in
    ...
    func startChunkedUpload(token: String) {
        ...
        self.createNextChunk(token: token, offset: 0, fileSize: totalFileSize, cipher: cipher, completion: {
            if let crypto = self.crypto, let cipher = cipher {
                self.account.getUserKeyPair(completion: { result in
                    ...
                    case .value(let userKeyPair):
                        do {
                            ...
                            self.completeUpload(token: token, encryptedFileKey: encryptedFileKey)
                ...
                self.completeUpload(token: token, encryptedFileKey: nil)
            }
            
        })
    }

    fileprivate func createNextChunk(token: String, offset: Int, fileSize: Int64, cipher: EncryptionCipher?, completion: @escaping () -> Void) {
        ...
        do {
            ...
            self.uploadNextChunk(token: token, chunk: uploadData, offset: offset, totalFileSize: fileSize, retryCount: 0, chunkCallback: { error in
                if let error = error {
                    self.callback?.onError?(error)
                    return
                }
                if lastBlock {
                    ...
                    self.createNextChunk(token: token, offset: newOffset, fileSize: fileSize, cipher: cipher, completion: completion)
                }
            }, callback: callback)
        } catch {
    ...
    fileprivate func uploadNextChunk(token: String, chunk: Data, offset: Int, totalFileSize: Int64, retryCount: Int, chunkCallback: @escaping (Error?) -> Void, callback: UploadCallback?) {
        if self.isCanceled {
            return
        }
        let requestUrl = serverUrl.absoluteString + apiPath + "/uploads/\(token)"
                                    ...
                                    case .success(let upload, _, _):
                                        ...
                                        upload.responseData { dataResponse in
                                            if let error = dataResponse.error {
                                                self.handleUploadError(error: error, token: token, chunk: chunk, offset: offset, totalFileSize: totalFileSize, retryCount: retryCount, chunkCallback: chunkCallback, callback: callback)
                                            } else {
                                                if self.checkMD5(result: dataResponse.result, localFileMD5: FileUtils.calculateMD5(chunk)) {
                                                    chunkCallback(nil)
                                                } else {
                                                 // MD5 check failed
                                                    self.handleUploadError(error: DracoonError.hash_check_failed, token: token, chunk: chunk, offset: offset, totalFileSize: totalFileSize, retryCount: retryCount, chunkCallback: chunkCallback, callback: callback)
                                                }
                                            }
                                        }
    ...
    fileprivate func handleUploadError(error: Error, token: String, chunk: Data, offset: Int, totalFileSize: Int64, retryCount: Int,
                           chunkCallback: @escaping (Error?) -> Void, callback: UploadCallback?) {
        if retryCount < DracoonConstants.CHUNK_UPLOAD_MAX_RETRIES {
            self.uploadNextChunk(token: token, chunk: chunk, offset: offset, totalFileSize: totalFileSize, retryCount: retryCount + 1, chunkCallback: chunkCallback, callback: callback)
        } else {
            chunkCallback(error)
        }
    }
    ...
    func completeUpload(token: String, encryptedFileKey: EncryptedFileKey?) {
        ...
        self.sendCompleteRequest(token: token, request: completeRequest, completion: { result in
            switch result {
            case .value(let node):
    ...
    fileprivate func sendCompleteRequest(token: String, request: CompleteUploadRequest, completion: @escaping DataRequest.DecodeCompletion<Node>) {
        do {
            let jsonBody = try encoder.encode(request)
            let requestUrl = serverUrl.absoluteString + apiPath + "/uploads/\(token)"
    ...
    func deleteUpload(token: String, completion: @escaping (Dracoon.Response) -> Void) {
        let requestUrl = serverUrl.absoluteString + apiPath + "/uploads/\(token)"
