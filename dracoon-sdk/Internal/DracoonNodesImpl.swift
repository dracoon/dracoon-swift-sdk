                ...
                if isEncrypted {
                    self.getFileKey(nodeId: nodeId, completion: { result in
                        ...
                        case .value(let encryptedFileKey):
                            self.startFileDownload(nodeId: nodeId, targetUrl: targetUrl, callback: callback, fileKey: encryptedFileKey, session: session)
                        }
                    })
                } else {
                    self.startFileDownload(nodeId: nodeId, targetUrl: targetUrl, callback: callback, fileKey: nil, session: session)
                }
            }
        })
    }
    
    fileprivate func startFileDownload(nodeId: Int64, targetUrl: URL, callback: DownloadCallback, fileKey: EncryptedFileKey?, session: SessionManager?) {
        
        let download = FileDownload(nodeId: nodeId, targetUrl: targetUrl, config: self.requestConfig, account: self.account, nodes: self,
                                    crypto: self.crypto, fileKey: fileKey, session: session, getEncryptionPassword: self.getEncryptionPassword)
